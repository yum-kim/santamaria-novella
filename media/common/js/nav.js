...
$(document).ready(function () {
    ...
    $(".navopen").css('height', documentHeight);
    $(".navopen").css('height', windowHeight);
    
    

  function menuClose() { //closes mobile menu
        $('.gnbIcon span, .gnbIcon span:before, .gnbIcon span:after').removeClass('open');
        setTimeout(function () {
            $('.gnbIcon span, .gnbIcon span:before, .gnbIcon span:after').removeClass('open');
        }, 100);
        $('.mobile').removeClass('navopen');
        $('.mobile .menu a').css('display','none');
    };
    ...
    function menuOpen() { //opens mobile menu
        ...
        $('.gnbIcon span, .gnbIcon span:before, .gnbIcon span:after').addClass('open');
        setTimeout(function () {
            $('.gnbIcon span, .gnbIcon span:before, .gnbIcon span:after').addClass('open');
        }, 250);
        $('.mobile').addClass('navopen');
        $('.mobile .menu a').css('display','block');
    };

    $('.gnbIcon').click(function () { //controls mobile menu behavior based on menu click event


        
        if ($('.gnbIcon span, .gnbIcon span:before, .gnbIcon span:after').hasClass('open')) {
            menuClose();
        } else {
            menuOpen();
        }
    ...
    //PC 네비효과 (1025px 이상에서만 호버 시 네비 쇼되는 효과)
